...
export const DiscrepancyBanner: React.FC<DiscrepancyBannerProps> = ({
  ...
  if (currentCase.status === "MISMATCH") {
    ...
    return (
      <div className="mb-4 px-4 py-3 rounded-xl bg-rose-50/90 border border-rose-200/90 dark:bg-rose-950/40 dark:border-rose-900/60 flex flex-wrap items-center justify-between gap-3 shadow-2xs animate-in fade-in">
        ...
        <div className="flex items-center space-x-2 shrink-0">
          {onOpenReview && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-white dark:bg-rose-950/80 border border-rose-300 dark:border-rose-800 text-rose-800 dark:text-rose-200 text-xs font-semibold hover:bg-rose-100 dark:hover:bg-rose-900/50 transition-colors cursor-pointer"
            >
              Review & Resolve
            </button>
          )}
  ...
  if (currentCase.status === "NEEDS_REVIEW") {
    ...
    return (
      <div className="mb-4 px-4 py-3 rounded-xl bg-amber-50/90 border border-amber-200/90 dark:bg-amber-950/40 dark:border-amber-900/60 flex flex-wrap items-center justify-between gap-3 shadow-2xs animate-in fade-in">
        <div className="flex items-center space-x-2.5 min-w-0">
        ...
        {onOpenReview ? (
          <button
            onClick={onOpenReview}
            className="px-3.5 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold shadow-xs flex items-center space-x-1.5 transition-colors cursor-pointer"
          >
            <span>Confirm & Review</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </button>
        ) : (
          <span className="px-3 py-1 rounded-lg bg-amber-200/70 text-amber-900 dark:bg-amber-900 dark:text-amber-200 text-xs font-medium shrink-0">
  ...
  if (currentCase.status === "PASS") {
    if (currentCase.promptInjectionDetected) {
      return (
        <div className="mb-4 px-4 py-3 rounded-xl bg-amber-50 border border-amber-300 dark:bg-amber-950/40 dark:border-amber-800 flex flex-wrap items-center justify-between gap-3 shadow-2xs animate-in fade-in">
